...
func updateStudents(c *gin.Context) {
	...
	c.JSON(http.StatusNotFound, gin.H{"error": "student not found"}) //สร้างjson  massage
}
...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		api.POST("/students", createStudents)
		api.PUT("/students/:id", updateStudents)
		api.DELETE("/students/:id", deleteStudents)
	}
