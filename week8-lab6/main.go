...
func getAllBooks(c *gin.Context) {
	...
	if yearQuery != "" {
		query += " WHERE year = $1"
		rows, err = db.Query(query, yearQuery)
	} else {
...
func createBook(c *gin.Context) {
	...
	err := db.QueryRow(
		...
		newBook.Title, newBook.Author, newBook.ISBN, newBook.Year, newBook.Price).Scan(&id, &createdAt, &updatedAt)
